...
public class KV_Team_OSD_GUI extends PApplet {
...
public void draw() {
  ...
  displayHorizon(PApplet.parseInt(MW_Pitch_Roll.arrayValue()[0])*10,PApplet.parseInt(MW_Pitch_Roll.arrayValue()[1])*10*-1);
  SimulateTimer();
  ShowCurrentThrottlePosition();
  if (PApplet.parseInt(confItem[GetSetting("S_DISPLAYRSSI")].value()) > 0)
    ShowRSSI(); 
  if (PApplet.parseInt(confItem[GetSetting("S_DISPLAYVOLTAGE")].value()) > 0) {
     ShowVolts(sVBat);
  }
...
public void CreateFontFile(){
 ...
 Output.flush(); // Writes the remaining data to the file
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "KV_Team_OSD_GUI" };
    if (passedArgs != null) {
